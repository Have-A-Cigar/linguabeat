...
**LinguaBeat** — веб-платформа для изучения русского языка через лицензированную музыку. Пользователь слушает трек, тапает незнакомое слово — оно добавляется в словарь с таймкодом и строфой, где встретилось. На повторении студент видит ту же строфу, слышит тот же контекст. Алгоритм **FSRS-5** расставляет сессии так, чтобы слово осталось в памяти при минимуме повторений.

Первый рынок: иностранные студенты российских вузов (РУДН, СПбГУ и др.).
Бизнес-модель: freemium, 250 руб./мес. за полный доступ.

Архитектура: трёхзвенная (клиент — API — данные), доменная логика SRS изолирована от веб-слоя.
...
|------|-----------|--------------------|
| **Frontend** | React 18 + TypeScript | Типобезопасность критична для модели слово/таймкод; удобная работа с интерактивным плеером. |
| **Сборка** | Vite | Быстрый HMR и нативный ESM ускоряют итерации. |
| **Стили** | TailwindCSS | Нет переключения контекста между CSS и компонентами, минимальный итоговый бандл. |
| **Backend** | FastAPI + Python 3.11 | ASGI для I/O-bound запросов, автогенерация OpenAPI, нативная Pydantic-валидация. Python совместим с `core_srs.py` без перевода. |
| **ORM** | SQLAlchemy 2.0 | Типизированный `Mapped[...]` API, async-движок, контроль SQL для SRS-выборок. |
| **БД** | PostgreSQL 15 | ACID для прогресса обучения, `JSONB` для субтитров (`lyrics_json`), индексы по `next_review`. |
| **Кэш** | Redis | Кэш треков, refresh-токены, rate limiting. |
| **Auth** | JWT + OAuth2 | Stateless — масштабируется горизонтально, OAuth2 password/bearer встроен в FastAPI. |
| **Контейнеризация** | Docker + Docker Compose | Одна команда поднимает весь стек локально и на сервере. |
| **CI/CD** | GitHub Actions | Бесплатно, lint + тесты + сборка + деплой по push в main. |

Принципы: stateless-бэкенд, contract-first API (OpenAPI), доменная логика SRS изолирована от веб-слоя и БД.
...
`get_context_card()` возвращает слово, перевод, строфу (`subtitle_text`) и таймкод (`timecode_sec`) — повторение проходит в том же музыкальном фрагменте, где студент впервые встретил слово.
...
Зависимости направлены строго внутрь. `domain/core_srs` ни от чего не зависит и покрыт юнит-тестами.
